# DESIGN_REVIEW1 — Grove - Time of Flight Distance Sensor VL53L0X

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveTimeOfFlightDistanceSensorVL53L0X`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `VL53L0X` · declared MPN `VL53L0X`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Sensor_Intro/)

This review is specific to the checked-in [board source](./GroveTimeOfFlightDistanceSensorVL53L0X.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveTimeOfFlightDistanceSensorVL53L0X.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveTimeOfFlightDistanceSensorVL53L0X.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | VL53L0X | VL53L0X | SDA, SCL, VDD, GND, ADDR, INT, pin7, pin8 |
...
- <trace#111515(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#111516(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#111517(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#111518(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#111519(from:.U2 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#111520(from:.U2 > .VOUT to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
