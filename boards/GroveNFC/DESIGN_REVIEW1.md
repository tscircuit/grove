# DESIGN_REVIEW1 — Grove NFC

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveNFC`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-NFC.html)

This review is specific to the checked-in [board source](./GroveNFC.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveNFC.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveNFC.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | PN532 | PN532 | SDA, SCL, VCC, GND, ADDR, INT |
...
- <trace#10990(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10991(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10992(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10993(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10994(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10995(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10996(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10997(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
