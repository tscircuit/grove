# DESIGN_REVIEW1 — Grove High Precision RTC

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveHighPrecisionRTC`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-High-Precision-RTC.html)

This review is specific to the checked-in [board source](./GroveHighPrecisionRTC.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveHighPrecisionRTC.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveHighPrecisionRTC.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- Footprint strings declared in source: `jlcpcb:C18723598`, `jlcpcb:C131334`, `jlcpcb:C14663`, `0603`.
...
- <trace#7251(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7252(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7253(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7254(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7255(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7256(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7257(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7258(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
