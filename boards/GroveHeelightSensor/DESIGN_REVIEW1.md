# DESIGN_REVIEW1 — Grove Heelight Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveHeelightSensor`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Heelight-Sensor.html)

This review is specific to the checked-in [board source](./GroveHeelightSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveHeelightSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveHeelightSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#7539(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7540(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7541(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7542(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7543(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7544(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7545(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7546(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
