# DESIGN_REVIEW1 — Grove Water Level Sensor 10CM

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveWaterLevelSensor10CM`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `LM393` · declared MPN `LM393`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Water-Level-Sensor-10CM-p-4443.html)

This review is specific to the checked-in [board source](./GroveWaterLevelSensor10CM.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveWaterLevelSensor10CM.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveWaterLevelSensor10CM.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#20507(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20508(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20509(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20510(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20511(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20512(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20513(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#20514(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
