# DESIGN_REVIEW1 — Grove - DS1307 RTC (Real Time Clock) for Arduino

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveDS1307RTCRealTimeClockForArduino`
...
This review is specific to the checked-in [board source](./GroveDS1307RTCRealTimeClockForArduino.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveDS1307RTCRealTimeClockForArduino.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveDS1307RTCRealTimeClockForArduino.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#2939(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2940(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2941(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2942(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2943(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2944(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2945(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2946(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
