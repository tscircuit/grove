# DESIGN_REVIEW1 — Grove Temp Humi Sensor SHT40

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveTempHumiSensorSHT40`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `SHT4x` · declared MPN `SHT4x`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Temp-Humi-Sensor-SHT40-p-5384.html)

This review is specific to the checked-in [board source](./GroveTempHumiSensorSHT40.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveTempHumiSensorSHT40.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveTempHumiSensorSHT40.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SHT4x | SHT4x | SDA, SCL, VDD, VSS, EP |
...
- Footprint strings declared in source: `jlcpcb:C2909890`, `jlcpcb:C131334`, `jlcpcb:C14663`, `jlcpcb:C5446`, `jlcpcb:C15849`, `sot23`, `0603`.
...
- <trace#17115(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17116(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17117(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17118(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17119(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17120(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17121(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17122(from:.U1 > .VSS to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
