# DESIGN_REVIEW1 — Grove - CO2 Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveCO2Sensor`
...
This review is specific to the checked-in [board source](./GroveCO2Sensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveCO2Sensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveCO2Sensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#5495(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5496(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5497(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5498(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5499(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5500(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5501(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5502(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
