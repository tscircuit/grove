# DESIGN_REVIEW1 — Grove 16 x 2 LCD Black on Red

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grove16X2LCDBlackOnRed`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-16-x-2-LCD-Black-on-Red.html)

This review is specific to the checked-in [board source](./Grove16X2LCDBlackOnRed.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grove16X2LCDBlackOnRed.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grove16X2LCDBlackOnRed.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | HD44780 | HD44780 | SDA, SCL, VCC, GND, ADDR, INT |
...
| U3 | simple_chip | HD44780 display panel | HD44780 | VCC, GND, SCL, SDA |
...
- <trace#1491(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1492(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1493(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1494(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1495(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1496(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1497(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1498(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
