# DESIGN_REVIEW1 — Grove LoRa Radio 433MHz

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveLoRaRadio433MHz`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** uart interface · communications · 5V · primary model `RFM95` · declared MPN `RFM95`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-LoRa-Radio-433MHz-p-2777.html)

This review is specific to the checked-in [board source](./GroveLoRaRadio433MHz.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveLoRaRadio433MHz.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveLoRaRadio433MHz.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | RFM95 | RFM95 | RX, TX, VCC, GND, CTS, RTS, pin7, pin8 |
...
- <trace#72093(from:.U1 > .RX to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72094(from:.U1 > .TX to:net.TX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72095(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72096(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72097(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72098(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72099(from:.R1 > .pin1 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72100(from:.R1 > .pin2 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
