# DESIGN_REVIEW1 — Grove - I2C High Accuracy Temp&Humi Sensor(SHT35)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveI2CHighAccuracyTempHumiSensorSHT35`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `SHT35` · declared MPN `SHT35`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Sensor_Intro/)

This review is specific to the checked-in [board source](./GroveI2CHighAccuracyTempHumiSensorSHT35.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveI2CHighAccuracyTempHumiSensorSHT35.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveI2CHighAccuracyTempHumiSensorSHT35.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SHT35 | SHT35 | SDA, ADDR, ALERT, SCL, VDD, NRESET, NC, GND, EP |
...
- Footprint strings declared in source: `jlcpcb:C90161`, `jlcpcb:C131334`, `jlcpcb:C14663`, `jlcpcb:C5446`, `jlcpcb:C15849`, `sot23`, `0603`.
...
- <trace#8250(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8251(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8252(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8253(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8254(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8255(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8256(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8257(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
