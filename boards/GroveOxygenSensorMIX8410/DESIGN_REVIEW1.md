# DESIGN_REVIEW1 — Grove Oxygen Sensor MIX8410

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveOxygenSensorMIX8410`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `MIX8410` · declared MPN `MIX8410`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Oxygen-Sensor-MIX8410-p-4697.html)

This review is specific to the checked-in [board source](./GroveOxygenSensorMIX8410.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveOxygenSensorMIX8410.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveOxygenSensorMIX8410.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MIX8410 | MIX8410 | SIG, VCC, GND, AUX |
...
- <trace#11873(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11874(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11875(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11876(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11877(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11878(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11879(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11880(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
