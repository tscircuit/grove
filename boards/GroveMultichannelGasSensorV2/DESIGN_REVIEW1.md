# DESIGN_REVIEW1 — Grove Multichannel Gas Sensor v2

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveMultichannelGasSensorV2`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `MiCS-6814` · declared MPN `MiCS-6814`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Multichannel-Gas-Sensor-v2-p-4569.html)

This review is specific to the checked-in [board source](./GroveMultichannelGasSensorV2.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveMultichannelGasSensorV2.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveMultichannelGasSensorV2.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MiCS-6814 | MiCS-6814 | SIG, VCC, GND, AUX |
...
- <trace#10943(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10944(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10945(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10946(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10947(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10948(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10949(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10950(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
