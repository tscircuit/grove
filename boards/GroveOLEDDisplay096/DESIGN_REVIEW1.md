# DESIGN_REVIEW1 — Grove OLED Display 0 96

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveOLEDDisplay096`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-OLED-Display-0-96.html)

This review is specific to the checked-in [board source](./GroveOLEDDisplay096.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveOLEDDisplay096.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveOLEDDisplay096.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#10425(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10426(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10427(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10428(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10429(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10430(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10431(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10432(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
