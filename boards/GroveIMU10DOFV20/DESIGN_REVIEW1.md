# DESIGN_REVIEW1 — Grove IMU 10DOF v2 0

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveIMU10DOFV20`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** digital interface · utility · 5V · primary model `MPU-9250` · declared MPN `MPU-9250`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-IMU-10DOF-v2-0.html)

This review is specific to the checked-in [board source](./GroveIMU10DOFV20.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveIMU10DOFV20.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveIMU10DOFV20.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#8618(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8619(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8620(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8621(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8622(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8623(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8624(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8625(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
