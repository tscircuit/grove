# DESIGN_REVIEW1 — Grove - Alcohol Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveAlcoholSensor`
...
This review is specific to the checked-in [board source](./GroveAlcoholSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveAlcoholSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveAlcoholSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#4632(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4633(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4634(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4635(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4636(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4637(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4638(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4639(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
