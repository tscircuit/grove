# DESIGN_REVIEW1 — Grove Gas Sensor MQ2

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveGasSensorMQ2`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `MQ-2` · declared MPN `MQ-2`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Gas-Sensor-MQ2.html)

This review is specific to the checked-in [board source](./GroveGasSensorMQ2.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveGasSensorMQ2.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveGasSensorMQ2.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MQ-2 | MQ-2 | SIG, VCC, GND, AUX |
...
- <trace#6863(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6864(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6865(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6866(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6867(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6868(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6869(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6870(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
