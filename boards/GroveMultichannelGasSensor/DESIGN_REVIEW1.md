# DESIGN_REVIEW1 — Grove - Multichannel Gas Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveMultichannelGasSensor`
...
This review is specific to the checked-in [board source](./GroveMultichannelGasSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveMultichannelGasSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveMultichannelGasSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#7813(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7814(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7815(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7816(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7817(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7818(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7819(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7820(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
