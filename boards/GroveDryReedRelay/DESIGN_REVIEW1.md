# DESIGN_REVIEW1 — Grove Dry Reed Relay

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveDryReedRelay`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Dry-Reed-Relay.html)

This review is specific to the checked-in [board source](./GroveDryReedRelay.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveDryReedRelay.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveDryReedRelay.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#6162(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6163(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6164(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6165(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6166(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6167(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6168(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6169(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
