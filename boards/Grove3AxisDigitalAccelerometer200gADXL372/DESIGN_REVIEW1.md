# DESIGN_REVIEW1 — Grove 3 Axis Digital Accelerometer 200g ADXL372

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grove3AxisDigitalAccelerometer200gADXL372`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `ADXL372` · declared MPN `ADXL372`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-3-Axis-Digital-Accelerometer-200g-ADXL372-p-4003.html)

This review is specific to the checked-in [board source](./Grove3AxisDigitalAccelerometer200gADXL372.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grove3AxisDigitalAccelerometer200gADXL372.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grove3AxisDigitalAccelerometer200gADXL372.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | ADXL372 | ADXL372 | SIG, VCC, GND, AUX |
...
- <trace#2486(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2487(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2488(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2489(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2490(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2491(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2492(from:.U2 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2493(from:.U2 > .VOUT to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
