# DESIGN_REVIEW1 — Grove AHT20 I2C Industrial grade temperature and humidity sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveAHT20I2CIndustrialGradeTemperatureAndHumiditySensor`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `AHT20` · declared MPN `AHT20`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-AHT20-I2C-Industrial-grade-temperature-and-humidity-sensor-p-4497.html)

This review is specific to the checked-in [board source](./GroveAHT20I2CIndustrialGradeTemperatureAndHumiditySensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveAHT20I2CIndustrialGradeTemperatureAndHumiditySensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveAHT20I2CIndustrialGradeTemperatureAndHumiditySensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | AHT20 | AHT20 | NC1, VDD, SCL, SDA, GND, NC2 |
...
- Footprint strings declared in source: `jlcpcb:C2757850`, `jlcpcb:C131334`, `jlcpcb:C14663`, `jlcpcb:C5446`, `jlcpcb:C15849`, `sot23`, `0603`.
...
- <trace#4152(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4153(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4154(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4155(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4156(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4157(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4158(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4159(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
