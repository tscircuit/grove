# DESIGN_REVIEW1 — Grove-Encoder

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveEncoder`
...
This review is specific to the checked-in [board source](./GroveEncoder.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveEncoder.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveEncoder.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#6083(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6084(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6085(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6086(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6087(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6088(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6089(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6090(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
