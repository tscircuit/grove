# DESIGN_REVIEW1 — Grove RS485

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveRS485`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-RS485-p-2924.html)

This review is specific to the checked-in [board source](./GroveRS485.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveRS485.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveRS485.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SN75176 | SN75176 | RX, TX, VCC, GND, CTS, RTS |
...
- <trace#13586(from:.J1 > .RX to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13587(from:.J1 > .TX to:net.TX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13588(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13589(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13590(from:.U1 > .RX to:net.RX_MCU) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13591(from:.U1 > .TX to:net.TX_MCU) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13592(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13593(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
