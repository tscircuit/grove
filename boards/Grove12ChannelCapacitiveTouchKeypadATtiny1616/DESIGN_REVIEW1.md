# DESIGN_REVIEW1 — Grove 12 Channel Capacitive Touch Keypad (ATtiny1616)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grove12ChannelCapacitiveTouchKeypadATtiny1616`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · input · 5V · primary model `ATtiny1616` · declared MPN `ATtiny1616`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Sensor_Intro/)

This review is specific to the checked-in [board source](./Grove12ChannelCapacitiveTouchKeypadATtiny1616.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grove12ChannelCapacitiveTouchKeypadATtiny1616.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grove12ChannelCapacitiveTouchKeypadATtiny1616.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | ATtiny1616 | ATtiny1616 | SIG, VCC, GND, AUX |
...
- <trace#1053(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1054(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1055(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1056(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1057(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1058(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1059(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1060(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
