# DESIGN_REVIEW1 — Grove - 2-Coil Latching Relay

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grove2CoilLatchingRelay`
...
This review is specific to the checked-in [board source](./Grove2CoilLatchingRelay.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grove2CoilLatchingRelay.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grove2CoilLatchingRelay.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#1873(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1874(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1875(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1876(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1877(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1878(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1879(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1880(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
