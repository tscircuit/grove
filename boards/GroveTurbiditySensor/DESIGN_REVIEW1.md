# DESIGN_REVIEW1 — Grove Turbidity Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveTurbiditySensor`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Turbidity-Sensor-p-4399.html)

This review is specific to the checked-in [board source](./GroveTurbiditySensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveTurbiditySensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveTurbiditySensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#15831(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15832(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15833(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15834(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15835(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15836(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15837(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15838(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
