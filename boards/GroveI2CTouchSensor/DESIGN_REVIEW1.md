# DESIGN_REVIEW1 — Grove I2C Touch Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveI2CTouchSensor`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · input · 5V · primary model `AT42QT1070` · declared MPN `AT42QT1070`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-I2C-Touch-Sensor-p-840.html)

This review is specific to the checked-in [board source](./GroveI2CTouchSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveI2CTouchSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveI2CTouchSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | AT42QT1070 | AT42QT1070 | SDA, SCL, VCC, GND, ADDR, INT |
...
- <trace#8177(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8178(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8179(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8180(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8181(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8182(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8183(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#8184(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
