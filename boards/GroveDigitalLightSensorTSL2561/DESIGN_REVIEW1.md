# DESIGN_REVIEW1 — Grove Digital Light Sensor TSL2561

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveDigitalLightSensorTSL2561`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Digital-Light-Sensor-TSL2561.html)

This review is specific to the checked-in [board source](./GroveDigitalLightSensorTSL2561.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveDigitalLightSensorTSL2561.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveDigitalLightSensorTSL2561.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | TSL2561 | TSL2561 | SIG, VCC, GND, AUX |
...
- <trace#2613(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2614(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2615(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2616(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2617(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2618(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2619(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2620(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
