# DESIGN_REVIEW1 — Grove Air Quality Sensor SGP41

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveAirQualitySensorSGP41`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Air-Quality-Sensor-SGP41-p-5687.html)

This review is specific to the checked-in [board source](./GroveAirQualitySensorSGP41.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveAirQualitySensorSGP41.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveAirQualitySensorSGP41.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#1117(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1118(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1119(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1120(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1121(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1122(from:.U1 > .VSS to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1123(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#1124(from:.U1 > .VDDH to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
