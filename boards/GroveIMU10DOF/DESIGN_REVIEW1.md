# DESIGN_REVIEW1 — Grove IMU 10DOF

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveIMU10DOF`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-IMU-10DOF-p-2386.html)

This review is specific to the checked-in [board source](./GroveIMU10DOF.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveIMU10DOF.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveIMU10DOF.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#61553(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61554(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61555(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61556(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61557(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61558(from:.R1 > .pin1 to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61559(from:.R1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#61560(from:.C_MOTION > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
