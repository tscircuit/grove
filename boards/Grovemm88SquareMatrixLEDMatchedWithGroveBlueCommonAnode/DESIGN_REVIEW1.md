# DESIGN_REVIEW1 — 38mm 8 8 square matrix LED matched with Grove Blue Common Anode

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grovemm88SquareMatrixLEDMatchedWithGroveBlueCommonAnode`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · display · 5V · primary model `MY9221` · declared MPN `MY9221`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/38mm-8-8-square-matrix-LED-matched-with-Grove-Blue-Common-Anode-p-4052.html)

This review is specific to the checked-in [board source](./Grovemm88SquareMatrixLEDMatchedWithGroveBlueCommonAnode.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grovemm88SquareMatrixLEDMatchedWithGroveBlueCommonAnode.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grovemm88SquareMatrixLEDMatchedWithGroveBlueCommonAnode.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MY9221 | MY9221 | SDA, SCL, VCC, GND, ADDR, INT |
...
| U3 | simple_chip | MY9221 display panel | MY9221 | VCC, GND, SCL, SDA |
...
- <trace#7419(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7420(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7421(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7422(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7423(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7424(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7425(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#7426(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
