# DESIGN_REVIEW1 — Grove - I2C Thermocouple Amplifier (MCP9600)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveI2CThermocoupleAmplifierMCP9600`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `MCP9600` · declared MPN `MCP9600`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Sensor_Intro/)

This review is specific to the checked-in [board source](./GroveI2CThermocoupleAmplifierMCP9600.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveI2CThermocoupleAmplifierMCP9600.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveI2CThermocoupleAmplifierMCP9600.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MCP9600 | MCP9600 | SDA, SCL, VDD, GND, ADDR, INT, pin7, pin8 |
...
- <trace#60348(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60349(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60350(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60351(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60352(from:.U2 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60353(from:.U2 > .VOUT to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60354(from:.U2 > .VIN to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#60355(from:.C2 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
