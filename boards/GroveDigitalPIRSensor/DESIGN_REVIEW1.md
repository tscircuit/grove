# DESIGN_REVIEW1 — Grove - Digital PIR Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveDigitalPIRSensor`
...
This review is specific to the checked-in [board source](./GroveDigitalPIRSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveDigitalPIRSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveDigitalPIRSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#5843(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5844(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5845(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5846(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5847(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5848(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5849(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#5850(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
