# DESIGN_REVIEW1 — Grove - Gas O₂ Sensor(MIX8410)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveGasOSensorMIX8410`
...
This review is specific to the checked-in [board source](./GroveGasOSensorMIX8410.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveGasOSensorMIX8410.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveGasOSensorMIX8410.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#6549(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6550(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6551(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6552(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6553(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6554(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6555(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6556(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
