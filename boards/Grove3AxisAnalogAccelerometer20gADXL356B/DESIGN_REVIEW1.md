# DESIGN_REVIEW1 — Grove 3 Axis Analog Accelerometer 20g ADXL356B

**Disposition:** NOT PRODUCTION READY
**Board directory:** `Grove3AxisAnalogAccelerometer20gADXL356B`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `ADXL356B` · declared MPN `ADXL356B`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-3-Axis-Analog-Accelerometer-20g-ADXL356B-p-4004.html)

This review is specific to the checked-in [board source](./Grove3AxisAnalogAccelerometer20gADXL356B.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/Grove3AxisAnalogAccelerometer20gADXL356B.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/Grove3AxisAnalogAccelerometer20gADXL356B.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | ADXL356B | ADXL356B | SIG, VCC, GND, AUX |
...
- <trace#2210(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2211(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2212(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2213(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2214(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2215(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2216(from:.U2 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#2217(from:.U2 > .VOUT to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
