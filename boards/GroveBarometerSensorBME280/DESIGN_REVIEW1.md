# DESIGN_REVIEW1 — Grove - Barometer Sensor(BME280)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveBarometerSensorBME280`
...
This review is specific to the checked-in [board source](./GroveBarometerSensorBME280.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveBarometerSensorBME280.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveBarometerSensorBME280.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#4527(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4528(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4529(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4530(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4531(from:.U1 > .GND1 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4532(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4533(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4534(from:.U1 > .VDDIO to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
