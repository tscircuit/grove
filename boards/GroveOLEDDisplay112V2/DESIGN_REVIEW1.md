# DESIGN_REVIEW1 — Grove OLED Display 1 12 V2

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveOLEDDisplay112V2`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-OLED-Display-1-12-V2.html)

This review is specific to the checked-in [board source](./GroveOLEDDisplay112V2.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveOLEDDisplay112V2.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveOLEDDisplay112V2.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#12252(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12253(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12254(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12255(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12256(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12257(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12258(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12259(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
