# DESIGN_REVIEW1 — Grove Lightning Sensor AS3935

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveLightningSensorAS3935`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `AS3935` · declared MPN `AS3935`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Sensor_Intro/)

This review is specific to the checked-in [board source](./GroveLightningSensorAS3935.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveLightningSensorAS3935.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveLightningSensorAS3935.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | AS3935 | AS3935 | SDA, SCL, VCC, GND, ADDR, INT, pin7, pin8 |
...
- <trace#69832(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69833(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69834(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69835(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69836(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69837(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69838(from:.R1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#69839(from:.R1 > .pin2 to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
