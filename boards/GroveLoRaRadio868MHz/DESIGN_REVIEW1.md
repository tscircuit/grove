# DESIGN_REVIEW1 — Grove LoRa Radio 868MHz

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveLoRaRadio868MHz`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-LoRa-Radio-868MHz.html)

This review is specific to the checked-in [board source](./GroveLoRaRadio868MHz.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveLoRaRadio868MHz.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveLoRaRadio868MHz.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#72463(from:.U1 > .RX to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72464(from:.U1 > .TX to:net.TX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72465(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72466(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72467(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72468(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72469(from:.R1 > .pin1 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#72470(from:.R1 > .pin2 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
