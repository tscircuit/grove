...
const GroveRGBLEDRing16WS2813Mini = () => (
  <board name={"GroveRGBLEDRing16WS2813Mini"} title={"Grove RGB LED Ring 16 WS2813 Mini"} width={"48mm"} height={"48mm"} borderRadius="1mm" solderMaskColor="blue" minViaEdgeToPadEdgeClearance="0.2mm" minViaPadDiameter="0.25mm">
    ...
    <GroveConnector kind="analog" powerVoltage={"5V"} connectToNets pcbX={-18} pcbY={0} pcbRotation={-90} schX={-10} schY={0} />
    {Array.from({ length: 16 }, (_, index) => {
      const name = `PIX${index + 1}`
      const angle = (index / 16) * Math.PI * 2 - Math.PI / 2
      const x = 3 + Math.cos(angle) * 16
      ...
      return <Fragment key={name}>
        ...
        <capacitor name={`C${index + 1}`} capacitance="100nF" manufacturerPartNumber="CC0603KRX7R9BB104" footprint="0603" maxDecouplingTraceLength={120 + "mm"} connections={{ pin1: "net.VCC", pin2: "net.GND" }} pcbX={capX} pcbY={capY} schX={-7.8 + index * 2.2} schY={4} schOrientation="vertical" />
      </Fragment>
    })}
