# DESIGN_REVIEW1 — Grove Temp Humi Sensor SHT41

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveTempHumiSensorSHT41`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Temp-Humi-Sensor-SHT41-p-5383.html)

This review is specific to the checked-in [board source](./GroveTempHumiSensorSHT41.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveTempHumiSensorSHT41.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveTempHumiSensorSHT41.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#14188(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14189(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14190(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14191(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14192(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14193(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14194(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#14195(from:.U1 > .VSS to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
