# DESIGN_REVIEW1 — Grove - SPDT Relay(30A)

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveSPDTRelay30A`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `HLS8L-DC3V-S-C` · declared MPN `HLS8L-DC3V-S-C`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Accessories_Intro/)

This review is specific to the checked-in [board source](./GroveSPDTRelay30A.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveSPDTRelay30A.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveSPDTRelay30A.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#17841(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17842(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17843(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17844(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17845(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17846(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17847(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#17848(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
