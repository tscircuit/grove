# DESIGN_REVIEW1 — Grove - I2C FM Receiver

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveI2CFMReceiver`
...
This review is specific to the checked-in [board source](./GroveI2CFMReceiver.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveI2CFMReceiver.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveI2CFMReceiver.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#52653(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52654(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52655(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52656(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52657(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52658(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52659(from:.R1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#52660(from:.R1 > .pin2 to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
