# DESIGN_REVIEW1 — Grove Uart Wifi

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveUartWifi`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Uart-Wifi-p-2495.html)

This review is specific to the checked-in [board source](./GroveUartWifi.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveUartWifi.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveUartWifi.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | ESP8285 | ESP8285 | RX, TX, VCC, GND, CTS, RTS |
...
- <trace#15141(from:.J1 > .RX to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15142(from:.J1 > .TX to:net.TX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15143(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15144(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15145(from:.U1 > .RX to:net.RX_MCU) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15146(from:.U1 > .TX to:net.TX_MCU) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15147(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15148(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
