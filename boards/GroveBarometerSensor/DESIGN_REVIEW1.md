# DESIGN_REVIEW1 — Grove Barometer Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveBarometerSensor`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `BMP180` · declared MPN `BMP180`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Barometer-Sensor.html)

This review is specific to the checked-in [board source](./GroveBarometerSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveBarometerSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveBarometerSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | BMP180 | BMP180 | SDA, SCL, VDD, GND, ADDR, INT |
...
- <trace#4466(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4467(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4468(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4469(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4470(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4471(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4472(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#4473(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
