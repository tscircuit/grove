# DESIGN_REVIEW1 — Grove - Servo

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveServo`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · actuator · 5V · primary model `SG90` · declared MPN `SG90`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Accessories_Intro/)

This review is specific to the checked-in [board source](./GroveServo.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveServo.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveServo.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SG90 | SG90 | SIG, VCC, GND, AUX |
...
| U3 | simple_chip | SG90 load stage | SG90 | POS, NEG, GND |
...
- <trace#15855(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15856(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15857(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15858(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15859(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15860(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15861(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#15862(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
