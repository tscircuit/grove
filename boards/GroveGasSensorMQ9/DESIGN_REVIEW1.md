# DESIGN_REVIEW1 — Grove Gas Sensor MQ9

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveGasSensorMQ9`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `MQ-9` · declared MPN `MQ-9`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Gas-Sensor-MQ9.html)

This review is specific to the checked-in [board source](./GroveGasSensorMQ9.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveGasSensorMQ9.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveGasSensorMQ9.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MQ-9 | MQ-9 | SIG, VCC, GND, AUX |
...
- <trace#6670(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6671(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6672(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6673(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6674(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6675(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6676(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6677(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
