# DESIGN_REVIEW1 — Grove NFC ST25DV64KC

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveNFCST25DV64KC`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · communications · 5V · primary model `ST25DV64` · declared MPN `ST25DV64`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-NFC-ST25DV64KC-p-5688.html)

This review is specific to the checked-in [board source](./GroveNFCST25DV64KC.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveNFCST25DV64KC.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveNFCST25DV64KC.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | ST25DV64 | ST25DV64 | SDA, SCL, VDD, GND, ADDR, INT, pin7, pin8 |
...
- <trace#80242(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80243(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80244(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80245(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80246(from:.U2 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80247(from:.U2 > .VOUT to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80248(from:.U2 > .VIN to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#80249(from:.C2 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
