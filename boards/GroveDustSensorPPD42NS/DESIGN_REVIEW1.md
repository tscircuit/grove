# DESIGN_REVIEW1 — Grove Dust Sensor PPD42NS

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveDustSensorPPD42NS`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `PPD42NS` · declared MPN `PPD42NS`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Dust-Sensor-PPD42NS.html)

This review is specific to the checked-in [board source](./GroveDustSensorPPD42NS.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveDustSensorPPD42NS.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveDustSensorPPD42NS.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | PPD42NS | PPD42NS | SIG, VCC, GND, AUX |
...
- <trace#6467(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6468(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6469(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6470(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6471(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6472(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6473(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#6474(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
