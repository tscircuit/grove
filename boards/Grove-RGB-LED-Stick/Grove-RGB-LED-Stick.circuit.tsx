...
const GroveRgbLedStick = () => (
  <board name={"GroveRgbLedStick"} title={"Grove - RGB LED Stick (10 WS2813 Mini)"} width={"89mm"} height={"20mm"} borderRadius="1mm" solderMaskColor="blue" minViaEdgeToPadEdgeClearance="0.2mm" minViaPadDiameter="0.25mm">
    ...
    <net name="EMITTER" />
    <net name="LOAD_NEG" />
    <GroveMountingHoles x={40.5} y={7} />
    {/* JLCPCB footprint imports: footprint="jlcpcb:C965558", footprint="jlcpcb:C131334", footprint="jlcpcb:C14663" */}
    <GroveConnector kind="digital" powerVoltage={"5V"} connectToNets pcbX={-38.5} pcbY={0} pcbRotation={-90} schX={-10} schY={0} />
    {Array.from({ length: 10 }, (_, index) => {
      const name = `PIX${index + 1}`
      const angle = (index / 10) * Math.PI * 2 - Math.PI / 2
      const x = -32.5 + index * 7.5
      const y = 0
      const capX = x
      const capY = y + 6
      return <Fragment key={name}>
        ...
        <capacitor name={`C${index + 1}`} capacitance="100nF" manufacturerPartNumber="CC0603KRX7R9BB104" supplierPartNumbers={{ jlcpcb: ["C14663"] }} footprint="0603" connections={{ pin1: "net.VCC", pin2: "net.GND" }} maxDecouplingTraceLength={178 + "mm"} pcbX={capX} pcbY={capY} schX={-7.8 + index * 2.2} schY={4} schOrientation="vertical" />
      </Fragment>
    })}
    ...
    <silkscreentext text={"RGB LED Stick (10 WS2813 Mini)"} pcbX={0} pcbY={8.5} fontSize="0.6mm" />
