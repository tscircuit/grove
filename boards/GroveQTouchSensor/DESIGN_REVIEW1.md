# DESIGN_REVIEW1 — Grove Q Touch Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveQTouchSensor`
...
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Q-Touch-Sensor.html)

This review is specific to the checked-in [board source](./GroveQTouchSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveQTouchSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveQTouchSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | AT42QT1070 | AT42QT1070 | SIG, VCC, GND, AUX |
...
- <trace#12320(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12321(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12322(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12323(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12324(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12325(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12326(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#12327(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
