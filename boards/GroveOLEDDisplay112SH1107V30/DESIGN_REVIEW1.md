# DESIGN_REVIEW1 — Grove OLED Display 1 12 SH1107 V3 0

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveOLEDDisplay112SH1107V30`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · display · 5V · primary model `SH1107` · declared MPN `SH1107`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-OLED-Display-1-12-SH1107-V3-0-p-5011.html)

This review is specific to the checked-in [board source](./GroveOLEDDisplay112SH1107V30.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveOLEDDisplay112SH1107V30.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveOLEDDisplay112SH1107V30.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SH1107 | SH1107 | SDA, SCL, VCC, GND, ADDR, INT |
...
| U3 | simple_chip | SH1107 display panel | SH1107 | VCC, GND, SCL, SDA |
...
- <trace#11486(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11487(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11488(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11489(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11490(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11491(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11492(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#11493(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
