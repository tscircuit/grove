# DESIGN_REVIEW1 — Grove - Long Range

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveLongRange`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** digital interface · communications · 5V · primary model `RFM95` · declared MPN `RFM95`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_network_module_intro/)

This review is specific to the checked-in [board source](./GroveLongRange.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveLongRange.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveLongRange.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
- <trace#71353(from:.U1 > .RX to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71354(from:.U1 > .TX to:net.TX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71355(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71356(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71357(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71358(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71359(from:.R1 > .pin1 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#71360(from:.R1 > .pin2 to:net.RX) /> is missing a name. Add a name prop to make the trace easier to identify.
