# DESIGN_REVIEW1 — Grove MQ3 Grove Gas Sensor

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveMQ3GroveGasSensor`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** analog interface · sensor · 5V · primary model `MQ-3` · declared MPN `MQ-3`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-MQ3-Grove-Gas-Sensor.html)

This review is specific to the checked-in [board source](./GroveMQ3GroveGasSensor.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveMQ3GroveGasSensor.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveMQ3GroveGasSensor.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | MQ-3 | MQ-3 | SIG, VCC, GND, AUX |
...
- <trace#10251(from:.J1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10252(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10253(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10254(from:.U1 > .SIG to:net.SIG) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10255(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10256(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10257(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#10258(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
