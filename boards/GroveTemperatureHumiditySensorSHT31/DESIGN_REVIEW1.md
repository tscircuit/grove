# DESIGN_REVIEW1 — Grove Temperature Humidity Sensor SHT31

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveTemperatureHumiditySensorSHT31`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · sensor · 5V · primary model `SHT31` · declared MPN `SHT31`
**Upstream reference:** [Seeed source](https://www.seeedstudio.com/Grove-Temperature-Humidity-Sensor-SHT31.html)

This review is specific to the checked-in [board source](./GroveTemperatureHumiditySensorSHT31.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveTemperatureHumiditySensorSHT31.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveTemperatureHumiditySensorSHT31.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SHT31 | SHT31 | SDA, ADDR, ALERT, SCL, VDD, NRESET, NC, GND, EP |
...
- Footprint strings declared in source: `jlcpcb:C80862`, `jlcpcb:C131334`, `jlcpcb:C14663`, `jlcpcb:C5446`, `jlcpcb:C15849`, `sot23`, `0603`.
...
- <trace#13838(from:.J1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13839(from:.J1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13840(from:.J1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13841(from:.J1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13842(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13843(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13844(from:.U1 > .VDD to:net.VDD) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#13845(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
