# DESIGN_REVIEW1 — Grove - OLED Yellow&Blue Display 0.96(SSD1315) - SPI/IIC -3.3V/5V

**Disposition:** NOT PRODUCTION READY
**Board directory:** `GroveOLEDYellowBlueDisplay096SSD1315SPIIIC33V5V`
**Implementation class:** board-local engineering draft
**Catalogue declaration:** i2c interface · display · 3.3V · primary model `SSD1315` · declared MPN `SSD1315`
**Upstream reference:** [Seeed source](https://wiki.seeedstudio.com/Grove_Accessories_Intro/)

This review is specific to the checked-in [board source](./GroveOLEDYellowBlueDisplay096SSD1315SPIIIC33V5V.circuit.tsx), its current generated circuit JSON, and its committed [PCB snapshot](./__snapshots__/GroveOLEDYellowBlueDisplay096SSD1315SPIIIC33V5V.circuit-pcb.snap.svg) / [schematic snapshot](./__snapshots__/GroveOLEDYellowBlueDisplay096SSD1315SPIIIC33V5V.circuit-schematic.snap.svg). A renderable snapshot is not fabrication approval.
...
| U1 | simple_chip | SSD1315 | SSD1315 | SDA, SCL, VCC, GND, ADDR, INT, pin7, pin8 |
...
| DISP1 | simple_chip | SSD1315 display panel | UNSPECIFIED-DISPLAY-GroveOLEDYellowBlueDisplay096SSD1315SPIIIC33V5V | VCC, GND, DATA |
...
- <trace#84790(from:.U1 > .SDA to:net.SDA) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84791(from:.U1 > .SCL to:net.SCL) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84792(from:.U1 > .VCC to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84793(from:.U1 > .GND to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84794(from:.C1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84795(from:.C1 > .pin2 to:net.GND) /> is missing a name. Add a name prop to make the trace easier to identify.
- <trace#84796(from:.R1 > .pin1 to:net.VCC) /> is missing a name. Add a name prop to make the trace easier to identify.
